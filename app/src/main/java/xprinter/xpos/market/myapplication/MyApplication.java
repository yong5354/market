...
public class MyApplication extends Application {
    ...
    private void initLocalApkList() {
        Observable.fromCallable(new Callable<List<PackageInfo>>() {
                    @Override
                    ...
                    public ObservableSource<List<BaseApkField>> apply(@NonNull List<PackageInfo> packageInfos) throws Exception {
                        for(PackageInfo p : packageInfos) {
                            if((p.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0) {
                                ...
                                if (p.packageName.equals("com.mxtech.videoplayer.ad")) {
                                    Log.e("FANGUOYONG", "signMD5:" + p.signatures.length + "," + p.signatures[0].toCharsString());
                                }
                            }
                        }
